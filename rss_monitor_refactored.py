...
class Config:
    ...
    MONITORING_INTERVAL_SECONDS = 1800  # 30 минут
...
class DatabaseManager:
    """Отдельный класс для управления БД (принцип SRP)"""
...
class TextProcessor:
    """Класс для обработки текста (принцип SRP)"""
    
    @staticmethod
    def clean_html(text: Optional[str]) -> str:
        """Очистка HTML тегов с обработкой ошибок"""
        if not text:
            return ""
...
class RSSParser:
    """Класс для парсинга RSS (принцип SRP)"""
    ...
    def parse_feed(self, feed_name: str, feed_url: str, keywords: List[str]) -> int:
        """Парсинг RSS фида с обработкой ошибок"""
        try:
...
class RSSMonitor:
    """Основной класс мониторинга RSS (рефакторинг - разделение ответственности)"""
    ...
    def get_active_feeds(self) -> List[Tuple[str, str]]:
        """Получение активных фидов с обработкой ошибок"""
        try:
            with self.db_manager.get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_active_keywords(self) -> List[str]:
        """Получение активных ключевых слов с обработкой ошибок"""
        try:
            with self.db_manager.get_connection() as conn:
                cursor = conn.cursor()
    ...
    def monitor_feeds(self):
        """Мониторинг фидов с улучшенной обработкой ошибок"""
        ...
        while self.monitoring:
            try:
                ...
                # Ожидание следующего цикла с возможностью прерывания
    ...
    def add_feed(self, name: str, url: str) -> bool:
        """Добавление RSS фида с валидацией"""
        try:
    ...
    def add_keyword(self, keyword: str) -> bool:
        """Добавление ключевого слова с валидацией"""
        try:
...
<html>
<head>
    <title>RSS News Monitor (Refactored)</title>
    <meta charset="utf-8">
    <style>
...
<body>
    <div class="header">
        <h1>RSS News Monitor (Refactored)</h1>
...
def add_feed():
    ...
    if monitor.add_feed(name, url):
        ...
        # В реальном приложении можно передать сообщение об ошибке через session
        return redirect(url_for('index'))

@app.route('/toggle_feed/<int:feed_id>')
def toggle_feed(feed_id):
    """Переключение статуса фида"""
    try:
        with monitor.db_manager.get_connection() as conn:
            cursor = conn.cursor()
...
# API endpoints с обработкой ошибок
@app.route('/api/news')
def api_news():
...
if __name__ == '__main__':
    print("RSS News Monitor Service (Refactored)")
